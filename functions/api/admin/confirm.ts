import { adminAuthorized, publicBaseUrl, recoveryEnabled } from "../../../functions-lib/env.ts";
...
export const onRequestPost: Handler = async ({ request, env, params, waitUntil }) => {
  ...
  const { db, payments, messaging } = buildContainer(env, request);
  ...
  if (purchase.reveal_token) {
    ...
    if (lead) {
      ...
      // With RECOVERY_ENABLED the chef gets the link on WhatsApp automatically
      // the moment the payment is confirmed — no operator copy-paste needed.
      // Best-effort: the panel still shows the link either way.
      if (recoveryEnabled(env) && transitioned) {
        const url = recovery_url;
        waitUntil(
          messaging
            .sendAccessLink(normalizeIlPhone(purchase.chef_phone), url)
            .catch((err) => console.error("confirm access-link send failed:", err)),
        );
      }
    }
  }
